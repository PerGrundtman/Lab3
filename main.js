...
/* We have to use the JavaScript bind() in order to get the same values of our (this) variables for all the callback methods used in this app */
  ...
  this.buttonReject.addEventListener('click', this.cancelSession.bind(this), false);
}
...
SIP_Communicator.prototype = {
  ...
  */
  ...
  cancelSession: function(){
    ...
    this.buttonTerminate.disabled=true;
    this.buttonAccept.disabled=true;
    this.buttonReject.disabled=true;
    this.session.reject();
    audioRing.pause();
    audioHangup.play();
  },
  ...
  */
  setSession: function(session){
    ...
    session.on('failed', function (request) {
      ...
      if (cause === SIP.C.causes.UNAVAILABLE){
      ...
      if (cause === SIP.C.causes.REJECTED){
        this.ua.message(caller, 'Im unavailable'); //send to 'caller'
        alert("callee is busy 2");
      }
      if (cause === SIP.C.causes.BUSY){
        this.ua.message(caller, 'Im buzy, or I just dont wanna speak!'); //send to 'caller'
        alert("callee is busy 3");
      }
